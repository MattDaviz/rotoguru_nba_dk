...
#BBall Ref Scraper--------------------
# create the days
x <- seq(as.Date("2016-10-26"), as.Date("2016-11-02"), by = "day")
...
data <- data[,c(2:27)]
colnames(data) <- c("Player","Team","Away","Opponent","Win/Loss","MP","FG","FGA","FG%","3P","3PA","3P%","FT","FTA","FT%","ORB","DRB","TRB","AST","STL","BLK","TOV","PF","PTS","Game.Score","Date")
...
#FDPs
data$FDPs <- data$PTS+(data$TRB*1.2)+(data$AST*1.5)+(data$STL*2)+(data$BLK*2)-data$TOV
#setwd("/users/mattd/Desktop")
#write.csv(data, "NBA 15-16.csv")
